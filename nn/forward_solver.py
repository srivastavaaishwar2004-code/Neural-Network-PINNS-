...
def dispersion_fun(ww, Vrr, vp, vs, h):
    ...
    # convert to complex64 as we get the complex values when Vrr is less than vs or vp
    ...
    # velocity ratios for each layer
    ...
    # propagate: E = F * E_halfspace 
    E = torch.bmm(F, E.unsqueeze(2)).squeeze(2)
...
if __name__ == '__main__':
    ...
    # test with TRUE Vr from your dataset
    ...
    print(f"True  Vr=274.3 → residual: {r1.item():.6f}  (should be ≈ 0)")
    print(f"Wrong Vr=200.0 → residual: {r2.item():.6f}  (should be ≠ 0)")
    print(f"Worse Vr=150.0 → residual: {r3.item():.6f}  (should be ≠ 0)")
