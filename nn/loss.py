...
def monotonic_loss(predicted_norm):
...
def physics_loss(predicted_norm, curves_norm, surrogate_net):
    ...
    for i in range(curves_norm.shape[1]):   # loop over 80 frequencies

        freq_n   = curves_norm[:, i, 0]     # normalized frequency (B,)
        Vr_obs_n = curves_norm[:, i, 1]     # observed normalized Vr (B,)

        # surrogate input: [vs1, vs2, den1, den2, h1, freq]
        surrogate_input = torch.cat([
            predicted_norm,
            freq_n.unsqueeze(1)
        ], dim=1)                           # (B, 6)

        # surrogate predicts Vr for this earth model at this frequency
        Vr_pred_n = surrogate_net(surrogate_input)  # (B, 1)

        residual = (Vr_pred_n.squeeze(1) - Vr_obs_n)**2
        residuals.append(residual)

    residuals = torch.stack(residuals, dim=1)   # (B, 80)
